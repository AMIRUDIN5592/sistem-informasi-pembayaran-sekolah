...
class PembayaranController extends Controller
{
    ...
    function get_tipe_tagihan($id){
        $tipe = Universe::tipe_tagihan($id);
        echo $tipe;
    }
    ...
    public function prosesBayar(Request $request, $nisn)
    {
        ...
        $pembayaran = Pembayaran::whereIn('bulan_bayar', $request->bulan_bayar)
            ...
            ->pluck('bulan_bayar')
            ->toArray();
            $id = 1;//SPP::where('id',$request->tahun_bayar)->first()->tahun;
            $tipe = Universe::tipe_tagihan($id);
            $jml = $tipe == 1 ? $request->jumlah_bayar : $request->total_bayar;
           
        if (!$pembayaran) {
            DB::transaction(function() use($request, $petugas, $jml) {
                foreach ($request->bulan_bayar as $bulan) {   
                    ...
                    Pembayaran::create([
                        ...
                        'tahun_bayar' => SPP::where('id',$request->tahun_bayar)->first()->tahun,
                        'id_tagihan' => $request->tahun_bayar,
                        'bulan_bayar' => $bulan,
                        'jumlah_bayar' => $jml
                    ]);
                }
