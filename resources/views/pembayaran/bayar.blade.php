					...
					<div class="row">
						<div class="col-lg-3">
							<div class="form-group">
								...
								<select required="" name="tahun_bayar" id="tahun_bayar" class="form-control select2bs4" onchange="updateNominal()">
...
<script>
	...
	function updateNominal() {
        var tahun_id = document.getElementById('tahun_bayar').value;
		$.ajax({
            url: "/pembayaran/get_tipe_tagihan/"+tahun_id, // Ganti dengan route yang sesuai di Laravel
            type: "GET",
            success: function(response) {
                var bulanBayar = document.getElementById('bulan_bayar').value;

				// Cek kondisi jika tipe tagihan bukan bulanan, sesuaikan total bayar
				if (response !== "bulanan") {
							document.getElementById('total_bayar').readOnly = false;
							
				} else {
					// Set readonly menjadi true jika bulanan
					document.getElementById('total_bayar').readOnly = true;
